...
con <- dbConnect(duckdb::duckdb(), "warehouse/waims_warehouse.duckdb")

# Get report date
report_date <- as.Date(params$report_date)

# Load latest data
latest_wellness <- dbGetQuery(con, sprintf("
  SELECT * FROM wellness
  WHERE CAST(date AS DATE) = DATE '%s'
", report_date))

latest_acwr <- dbGetQuery(con, sprintf("
  SELECT * FROM acwr
  WHERE CAST(date AS DATE) = DATE '%s'
", report_date))
...
if (nrow(poor_sleepers) > 0) {
  cat("⚠️ **", nrow(poor_sleepers), "athletes** had inadequate sleep (<6.5 hours)\n\n")
  ...
  cat("\n\n📚 **Research:** Sleep <6.5 hours increases injury risk 1.7x (Milewski et al. 2014)\n")
  cat("\n**Recommendation:** Consider modified training load or extra recovery\n")
  
} else {
  cat("✅ **All athletes** had adequate sleep (≥6.5 hours)\n\n")
...
if (nrow(high_risk) > 0) {
  cat("🚨 **", nrow(high_risk), "athletes** showing elevated injury risk\n\n")
  ...
  cat("✅ **No athletes** showing high injury risk indicators\n")
}
```
...
# Count by status
green_count <- sum(readiness$status == "🟢 Ready")
yellow_count <- sum(readiness$status == "🟡 Monitor")
red_count <- sum(readiness$status == "🔴 At Risk")

cat("**Team Readiness Status:**\n\n")
cat("- 🟢 Ready (≥80):", green_count, "athletes\n")
cat("- 🟡 Monitor (60-79):", yellow_count, "athletes\n")
cat("- 🔴 At Risk (<60):", red_count, "athletes\n\n")
