...
Once workflow files are in your repo under `.github/workflows/`, GitHub runs them on schedule automatically.
...
# Evidence monitor workflow (generated by the script)
python research_monitor.py --github-action > .github/workflows/research_monitor.yml

# Model retrain workflow (copy from repo root)
cp retrain_models.yml .github/workflows/retrain_models.yml

# Commit and push — GitHub activates them immediately
git add .github/
git commit -m "Add automated workflows"
git push
```

Then go to your repo → **Actions** tab to confirm both workflows appear.
To trigger manually: Actions → select workflow → **Run workflow** button.

### Schedule

| Workflow | Schedule | What it does |
|---|---|---|
| `retrain_models.yml` | Sunday 6am UTC | Retrains RF model + readiness scorer, commits updated pkl files |
| `research_monitor.yml` | Monday 8am UTC | PubMed + RSS search, commits new papers to research_log.json |

### Do I need to do anything after setup?
No. GitHub handles the scheduling. The dashboard reads updated models and research log on every page load — no restart needed.

### `GITHUB_TOKEN`
Both workflows use `${{ secrets.GITHUB_TOKEN }}` to push commits back. This is **automatically provided by GitHub** — you do not need to create or configure it.
