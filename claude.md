\# CLAUDE.md



\## Project

WAIMS is the main Python project in this repo.

It is a Python + Streamlit athlete monitoring dashboard using synthetic demo data in V1.



\## Main files

\- `dashboard.py`

\- `coach\_command\_center.py`

\- `train\_models.py`

\- `model\_validation.py`

\- `data\_quality.py`

\- `README.md`

\- `WAIMS\_SESSION\_HANDOFF.md`

\- `WAIMS\_GLOBAL\_CONTEXT.md`



\## Stable rules

\- Keep coach-facing outputs simple and practical.

\- Keep sport scientist outputs more technical.

\- Athlete-facing views must show only the logged-in athlete's own data.

\- Athlete voice is keyword-safe V1 only, not open-ended natural-language reasoning.

\- Do not casually change evidence-based thresholds.

\- Prefer editing real source files instead of generated outputs.

\- `WAIMS\_Coach\_Overview.pdf` should remain a true one-pager.

\- `WAIMS\_SportScientist\_Overview.pdf` can be multi-page.



\## Compacting

When compacting, preserve:

\- current task

\- files inspected or changed

\- important commands

\- decisions already made

\- blockers or open questions



Do not preserve in detail:

\- long logs

\- repeated repo descriptions

\- unrelated exploration

\- rejected approaches unless still relevant



